...
function parseKeysHtml(html) {
  ...
  return rows.map((cells, i) => {
    ...
    // Disabled = class="disabled" on the <tr> row OR on the LicenseKeyLink <a>
    const disabled = isDisabledRow(rowAttrs[i]) ||
      /class="[^"]*\bdisabled\b[^"]*"/.test(cells[1] || '');
...
// ── Revenue parser ──────────────────────────────────────────────────────────
// Parses the Revenue/Points tab: summary labels + Attributed Revenue table
function parseRevenueHtml(html) {
  const lbl = (id) => {
    const m = html.match(new RegExp(`id="${id}"[^>]*>([^<]+)<`, 'i'));
    return m ? htmlDecode(m[1]).trim() : '';
  };
  const revenueBalance = lbl('Main_CustomerEditPoints_lblPoints');
  const previousAnnual = lbl('Main_CustomerEditPoints_lblPrevPointsAnnual');

  // Parse the Attributed Revenue table (CustomerEditEstimatedRevenue_dg)
  const attributed = [];
  const tblIdx = html.indexOf('id="Main_CustomerEditPoints_CustomerEditEstimatedRevenue_dg"');
  if (tblIdx >= 0) {
    const tblStart = html.lastIndexOf('<table', tblIdx);
    const tblEnd = html.indexOf('</table>', tblIdx);
    if (tblStart >= 0 && tblEnd >= 0) {
      const tblHtml = html.substring(tblStart, tblEnd + 8);
      const rowMatches = [...tblHtml.matchAll(/<tr\b[^>]*>([\s\S]*?)<\/tr>/gi)];
      for (let i = 1; i < rowMatches.length; i++) { // skip header
        const cells = [...rowMatches[i][1].matchAll(/<td[^>]*>([\s\S]*?)<\/td>/gi)].map(m =>
          htmlDecode(m[1].replace(/<[^>]+>/g, '')).trim().replace(/\s+/g, ' ')
        );
        if (cells.length >= 4) {
          attributed.push({ year: cells[0], direct: cells[1], indirect: cells[2], total: cells[3] });
        }
      }
    }
  }

  return { revenueBalance, previousAnnual, attributed };
}
...
async function fetchPartner360(partnerId) {
  ...
  const base = {
    ...
    solutionProvider: checkboxChecked('Main_CustomerEditParent_cbSolutionProvider'),
    highPotential:    checkboxChecked('Main_CustomerEditGeneral_cbHighPotential'),
    salesRep:         labelText('Main_CustomerEditGeneral_lblSalesRep'),
    startDate:        labelText('Main_CustomerEditGeneral_lblStartDate'),
    creditLimit:      inputVal('Main_CustomerEditCreditLimit_tbCreditLimit'),
    sellModel:        selectedOption('Main_CustomerEditGeneral_ddlPartnerSellOptions'),
    sipTrunkProvider: selectedOption('Main_CustomerEditGeneral_ddlPartnerSipTrunkOption'),
    vat:              inputVal('Main_CustomerEditBilling_tbVat'),
    paymentMethod:    selectedOption('Main_CustomerEditBilling_ddlPaymentMethod'),
    currency:         selectedOption('Main_CustomerEditBilling_ddlCurrency'),
    fetchedAt: Date.now()
  };
  ...
  for (const tab of tabs) {
    try {
      ...
      } else if (tab.key === 'points') {
        // Specialist: Revenue tab has summary labels + attributed revenue table
        tabData.revenue = parseRevenueHtml(allHtml);
        console.log(`[revenue] balance=${tabData.revenue.revenueBalance}, ${tabData.revenue.attributed.length} years`);
      } else {
  ...
  for (const level of MVP_LEVELS) {
    try {
      ...
      for (let page = 2; page <= cap; page++) {
        broadcast('partner360_status', `Loading ${level.name} partners page ${page}/${cap}…`);
        try {
          const nextFields = {
            ...currentFields,
            '__ASYNCPOST':           'true',
...
// ── Caller-ID match via ONYX server ─────────────────────────────────────────
async function matchCaller(phone) {
  try {
    const onyxBase = await getOnyxUrl();
    const r = await fetch(`${onyxBase}/api/match-caller?phone=${encodeURIComponent(phone)}`);
    return r.json();
  } catch (e) {
    console.warn('[ONYX] matchCaller failed:', e.message);
    return { matched: false, callerDigits: phone, candidates: [] };
  }
}

// ── Fetch individual key detail page (/key/edit.aspx?i=<keyId>) ─────────────
async function fetchKeyDetail(keyId) {
  const url = `https://staff.3cx.com/key/edit.aspx?i=${encodeURIComponent(keyId)}`;
  console.log(`[ONYX] Fetching key detail: ${url}`);
  const resp = await fetch(url, { credentials: 'include' });
  if (!resp.ok) throw new Error(`Key detail fetch failed: ${resp.status}`);
  const html = await resp.text();

  // Parse fields from the key detail page
  const lbl = (id) => {
    const m = html.match(new RegExp(`id="${id}"[^>]*>([\\s\\S]*?)<\\/(?:span|a|div|b)>`, 'i'));
    if (!m) return '';
    let v = htmlDecode(m[1].replace(/<[^>]+>/g, '')).trim();
    // Filter out placeholder values
    if (/^<none>$/i.test(v) || v === 'n/a' || v === '&lt;none&gt;') return '';
    return v;
  };
  const inp = (id) => {
  ...
  // Parse activations table for FQDN — the static Phone System labels are often <none>
  // but the activations table has the real FQDN from GetKeyInfo/Activation rows
  let lastFqdn = '';
  let lastActivationDate = '';
  const actIdx = html.indexOf('id="Main_activations_ActivationsDataGrid"');
  if (actIdx >= 0) {
    const actStart = html.lastIndexOf('<table', actIdx);
    const actEnd = html.indexOf('</table>', actIdx);
    if (actStart >= 0 && actEnd >= 0) {
      const actHtml = html.substring(actStart, actEnd + 8);
      const actRows = [...actHtml.matchAll(/<tr\b[^>]*>([\s\S]*?)<\/tr>/gi)];
      for (let i = 1; i < actRows.length; i++) { // skip header
        const cells = [...actRows[i][1].matchAll(/<td[^>]*>([\s\S]*?)<\/td>/gi)]
          .map(m => htmlDecode(m[1].replace(/<[^>]+>/g, '')).trim().replace(/\s+/g, ' '));
        if (cells.length < 9) continue;
        const fqdn = cells[8];
        if (fqdn && fqdn !== '&nbsp;' && fqdn.length > 2 && !lastFqdn) {
          lastFqdn = fqdn;
        }
        // First non-empty date from an Activation row = last activation
        const type = cells[1];
        if (/Activation/i.test(type) && cells[0] && !lastActivationDate) {
          lastActivationDate = cells[0];
        }
      }
    }
  }

  // Static Phone System labels (may be <none> — activations table is fallback)
  const staticFqdn = lbl('Main_phoneSystem_lblFQDN');
  const staticExt  = lbl('Main_phoneSystem_lblNumberOfExtensions');

  return {
    keyId,
    licenseKey:       lbl('Main_keyDetails_lblLicenseKey'),
    product:          lbl('Main_keyDetails_lblProductDescription'),
    billingType:      lbl('Main_keyDetails_lblAdditionalDescription'),
    facilityId:       lbl('Main_keyDetails_lblFacilityId'),
    issuedTo:         lbl('Main_keyDetails_hlIssuedTo') || lbl('Main_keyDetails_lblIssuedTo'),
    purchaseDate:     lbl('Main_keyDetails_lblPurchaseDate'),
    fqdn:             staticFqdn || lastFqdn,
    extensions:       staticExt,
    deployedAs:       lbl('Main_phoneSystem_lblDeployedAs'),
    registration:     inp('Main_regCustomer_tbCompany'),
    reseller:         lbl('Main_regCustomer_hlResellerCompany'),
    activations:      lbl('Main_activations_NavigatorControl_lblRecordCount'),
    lastActivation:   lastActivationDate,
  };
}

// ── Push scraped data to ONYX server (fire-and-forget proxy) ────────────────
// The extension is the data collector; ONYX is the data store + AI platform.
// If the server is unreachable, the extension still works fine — data lives
// in chrome.storage.local. The push is a bonus that feeds the SPA.

async function getRepEmail() {
  try {
    const health = await checkSessionHealth();
    return health.sessionInfo?.email || null;
  } catch { return null; }
}

async function pushPartnerListToOnyx(partners) {
  try {
    const onyxBase = await getOnyxUrl();
    const repEmail = await getRepEmail();
    if (!repEmail || !partners?.length) return;
    console.log(`[ONYX] Pushing ${partners.length} partners to ${onyxBase}/api/ingest/erp`);
    ...
    if (r.ok) console.log(`[ONYX] Ingest OK: ${body.counts?.partners || 0} partners stored`);
    else console.warn(`[ONYX] Ingest failed: ${r.status} ${body.error || ''}`);
  } catch (e) {
    console.warn('[ONYX] Push partner list failed (non-fatal):', e.message);
  }
}
...
async function pushPartnerDetailToOnyx(partnerId, detail) {
  try {
    const onyxBase = await getOnyxUrl();
    const repEmail = await getRepEmail();
    if (!repEmail || !partnerId || !detail) return;
    const r = await fetch(`${onyxBase}/api/ingest/erp/partner-detail`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ repEmail, partnerId: String(partnerId), detail }),
    });
    if (r.ok) console.log(`[ONYX] Detail pushed for partner ${partnerId}`);
    else console.warn(`[ONYX] Detail push failed for ${partnerId}: ${r.status}`);
  } catch (e) {
    console.warn(`[ONYX] Push detail failed for ${partnerId} (non-fatal):`, e.message);
  }
}
...
chrome.runtime.onMessage.addListener((msg, _sender, sendResponse) => {
  const dispatch = {
    ...
    FETCH_PARTNER360:   async () => {
      ...
      // Merge with existing keysSummary so we don't lose enrichment data
      const ksKey = `ks_${msg.partnerId}`;
      const ksData = await chrome.storage.local.get(ksKey);
      const ks = ksData[ksKey];
      const detail = ks ? { ...result, keysSummary: ks } : result;
      pushPartnerDetailToOnyx(msg.partnerId, detail);
      return result;
    },
    ...
    FETCH_PARTNER_LIST: async () => {
      ...
      // Push to ONYX server (fire-and-forget)
      pushPartnerListToOnyx(result);
      return result;
    },
    FETCH_PARTNER_LIST_FILTERED: async () => {
      const result = await fetchPartnerListFiltered(msg.levelId);
      // Don't replace the full list on ONYX — filtered lists are subsets.
      // But the extension caches for its own use.
      return result;
    },
    FETCH_KEYS_SUMMARY: async () => {
      const result = await fetchKeysSummary(msg.partnerId);
      if (result) {
        // Cache in chrome.storage.local so FETCH_PARTNER360 can merge it
        await chrome.storage.local.set({ [`ks_${msg.partnerId}`]: result });
        pushPartnerDetailToOnyx(msg.partnerId, { keysSummary: result });
      }
      return result;
    },
    ...
    FETCH_KEY_DETAIL:   () => fetchKeyDetail(msg.keyId),
